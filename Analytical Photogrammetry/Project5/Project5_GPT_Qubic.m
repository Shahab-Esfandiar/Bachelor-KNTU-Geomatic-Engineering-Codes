...
%project_5:GP transformation_Cubic
...
%MatrixCalc
for i=1:length(X)
    Ax(i,:)=[1,X(i),Y(i),X(i)*Y(i),X(i)^2,Y(i)^2,X(i)^2*Y(i),X(i)*Y(i)^2,X(i)^3,Y(i)^3];
    Lx(i,1)=E(i);
end
...
%RMSWCalc
for i=1:length(Xc)
    Ax_ch(i,:)=[1,Xc(i),Yc(i),Xc(i)*Yc(i),Xc(i)^2,Yc(i)^2,Xc(i)^2*Yc(i),Xc(i)*Yc(i)^2,Xc(i)^3,Yc(i)^3];
    Lx_c(i,1)=Ec(i);
end
